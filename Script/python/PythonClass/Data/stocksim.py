...
HTTP_ADDR = ("localhost",31337)

history_file = "dowstocks.dat"
...
class HTTPThread(threading.Thread):
    def __init__(self):
        ...
        self.server = BaseHTTPServer.HTTPServer(HTTP_ADDR,StockHTTPHandler)
        self.server.thr = self
    def update(self,record):
        self.access.acquire()
        self.current[record[0]] = record
        self.access.release()
...
class StockHTTPHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    def do_GET(self):
        if not self.path.startswith('/stocks.csv'):
            self.send_response(404)
            self.send_header('Content-type','text/html')
            self.end_headers()
            self.wfile.write("<html><body>Error 404. Not Found</body></html>\n")
            return
        ...
        self.end_headers()
        thr.access.acquire()
        response = thr.current.values()
        thr.access.release()
        response.sort(key=lambda t: t[0])
        for r in response:
            if not names or r[0] in names:
                self.wfile.write(csv_record(r)+"\n")
...
m.register(LogPrinter("stocklog.dat"))

try:
    http_handler = HTTPThread()
    m.register(http_handler)
    http_handler.start()
except StandardError,e:
    print "Couldn't start HTTP server", e
